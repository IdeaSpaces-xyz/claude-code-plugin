...
function inspectFrontmatterSyntax(content) {
  ...
  const source = block.lines.slice(1, block.endLineIndex).join("\n");
  const doc = (0, import_yaml.parseDocument)(source, { uniqueKeys: false });
...
function startsFrontmatter(content) {
  return content.startsWith(`${DELIM2}
`) || content.startsWith(`${DELIM2}\r
`);
}
