...
var KIND_REFERENCES = Object.freeze({
...
function agreementTitle(name) {
  const title = `Agreement \u2014 ${name}`;
  return isSafeAgentName(name) ? title : JSON.stringify(title);
}
var PROMPTS_NOTE = `> Every section below is a prompt, not content. Draw it out in conversation \u2014 cases, in the owner's
> own words \u2014 and replace it. Thin is honest; invented is not.`;
var AGREEMENT_TAIL = (page) => `## Alone, and brought back
...
`;
function knowledgeAgreementMd(name) {
  return `---
...
function agentAgreementMd(agentName) {
  return `---
...
var createCommand = {
  ...
  async run(args2, flags2, global2) {
    ...
    if (shape === "old-shape") {
      output.error(`${describeTarget(targetDir, name)} has an \`_agent/\` in the legacy shape (always.md / rules.md / soul.md). Migration is not yet automated; move their content into \`_agent/agreement.md\` by hand.`);
      return 5;
    }
...
function detectShape(inspection) {
  if (!inspection.exists)
    return "greenfield";
  if (inspection.hasAgreement || inspection.hasFoundation)
    return "complete";
...
function legacyGitUrl2(apiUrl, namespace, slug) {
...
Git needs a \`user.name\` to commit. Run \`git config --local user.name "Your Name"\` and retry.`;
    ...
    const remoteUrl = repo.root_node_id ? canonicalGitUrl(config.apiUrl, repo.root_node_id) : legacyGitUrl2(config.apiUrl, namespace, repo.slug);
...
${push2.stderr}${hint}`);
    ...
    const webUrl = repo.root_node_id ? canonicalRepoUrl(config.apiUrl, repo.root_node_id) : legacyWebUrl(config.apiUrl, namespace, repo.slug);
    output.result({
      repo_id: repo.repo_id,
...
function gitRead(cwd, args2) {
  ...
  if (result.error || result.status !== 0) {
    throw new Error(result.error?.message || result.stderr.trim() || `git ${args2.join(" ")} failed`);
  }
  return result.stdout;
}
function exactRemoteHead(cwd, branch) {
  const output = gitRead(cwd, ["ls-remote", "--exit-code", "origin", `refs/heads/${branch}`]);
  const lines = output.trim().split("\n").filter(Boolean);
  if (lines.length !== 1)
    throw new Error(`origin has no exact refs/heads/${branch} branch`);
...
function noteDisclosure(content, position) {
  const syntax = inspectFrontmatterSyntax(content);
  if (syntax.status === "malformed") {
  ...
  if (rawName !== void 0 && typeof rawName !== "string") {
    throw new Error("The committed Note frontmatter name must be a string");
  }
  if (rawSummary !== void 0 && typeof rawSummary !== "string") {
...
async function runMapSelection(args2, flags2, _global, output) {
  ...
  try {
    ...
    output.error(error instanceof UnauthorizedError ? "Session expired. Run `ideaspaces login`." : error instanceof Error ? error.message : String(error));
    return 1;
  }
}
...
function planFolder(dir) {
  ...
  return {
    ...
    kind: "folder",
    modes: {
      link: inside ? origin ? { available: true, origin } : { available: false, origin: null, reason: "no `origin` remote \u2014 cannot tell which Space it belongs to" } : { available: false, origin: null, reason: "not a git repository" }
    },
    logged_in: loadConfig() !== null
  };
}
async function planSpace(address, output) {
  ...
  if (loggedIn) {
    try {
      const me = await fetchAuthMe(auth);
  ...
  let source = null;
  try {
    const read2 = await optionalAuthRead(auth, (config) => getSpace(config, rootNodeId));
    source = read2.value;
  } catch (err) {
    if (!/→ (?:401|403|404):/.test(err instanceof Error ? err.message : String(err)))
      throw err;
  }
...
function renderPlan(plan) {
  ...
  if (plan.kind === "space") {
    ...
    lines.push("", `Collaborate on this Space (clone) \u2014 same identity, full history${c.available ? "" : " \u2014 not available"}`, `  fetch: ${c.fetch}`, `  push:  ${c.push}`);
    ...
    lines.push("", `Bind this clone to its Space (link)${l.available ? "" : " \u2014 not available"}`, l.available ? `  origin: ${l.origin}` : `  ${l.reason}`);
  }
  const available = Object.entries(plan.modes).filter(([, m]) => m.available).map(([mode]) => mode);
...
${renderPlan(plan)}`);
  ...
  switch (mode) {
    case "clone":
      return cloneCommand.run([plan.address, ...rest], flags2, global2);
    case "fork":
      return forkCommand.run([plan.address, ...rest], flags2, global2);
    case "link":
      return linkCommand.run([plan.address, ...rest], flags2, global2);
  }
}
var getCommand = {
  name: "get",
  ...
  async run(args2, flags2, global2) {
    ...
    const as = flags2.as;
    if (as !== void 0 && !GET_MODES.includes(String(as))) {
      output.error(`--as must be one of ${GET_MODES.join(", ")}`);
      return 1;
    }
    if (as !== void 0 && !global2.yes) {
...
${renderPlan(plan)}`);
    ...
    return execute(as, plan, args2, flags2, global2, output);
  }
};
...
function planIntegrate(record, upstream, requested) {
  ...
  if (requested === "source") {
    if (!hasSource)
      return { error: "This checkout has no maintained source; nothing to integrate from source." };
    return { from: "source", reason: "maintained source, three-way (asked)" };
  }
  if (requested === "remote") {
    if (!upstream)
      return { error: "No upstream configured for the current branch; nothing to integrate from the remote." };
    return { from: "remote", reason: `upstream ${upstream} (asked)` };
  }
  if (unpublished)
    return { from: "source", reason: "an unpublished fork with a maintained source" };
  if (upstream && hasSource) {
    return { from: "remote", also: "source", reason: `upstream ${upstream}; this fork also has a maintained source` };
  }
  if (upstream)
    return { from: "remote", reason: `upstream ${upstream}` };
  if (hasSource)
    return { from: "source", reason: "a maintained source and no upstream" };
  return {
    error: "Nothing to integrate from: no upstream on the current branch and no maintained source recorded. `ideaspaces get` binds a clone; `ideaspaces publish` gives a fork a remote."
  };
}
var integrateCommand = {
  ...
  async run(_args, flags2, global2) {
    const output = createOutput(global2);
    const requested = flags2.from;
    if (requested !== void 0 && requested !== "remote" && requested !== "source") {
    ...
    const plan = planIntegrate(findSpaceFor(root), remoteState(root).upstream, requested);
    if ("error" in plan) {
    ...
    if (plan.from === "remote") {
      const pullFlags = global2.yes ? flags2 : { ...flags2, "dry-run": true };
      return pullCommand.run([], pullFlags, global2);
    }
    return updateCommand.run([], flags2, global2);
  }
};
...
function standingForSelector(standings, selector) {
  const needle = ("username" in selector ? selector.username : selector.email).toLowerCase();
  return standings.find((standing) => "username" in selector ? standing.username?.toLowerCase() === needle : standing.email?.toLowerCase() === needle);
}
function rejectLegacyShare(sub, output) {
  const replacements = {
...
function invitationDeliverySummary(invite) {
  if (invite.delivery_status === "sent")
    return "";
  if (invite.delivery_status === "sending")
    return "; delivery in progress";
  const state = invite.delivery_status === "failed" ? `; delivery failed${invite.delivery_error ? ` (${invite.delivery_error})` : ""}` : "; delivery status unknown";
  if (invite.can_resend)
    return `${state}; resend available`;
...
async function resolveTarget(repoUrl, config, output) {
  if (repoUrl) {
    try {
  ...
  try {
    ...
    output.error("Not inside a repository. Run this from a clone, or name one: --repo <url>");
    return null;
  }
  const binding = await resolveSpaceBinding(root, config);
  if ("rootNodeId" in binding)
...
async function shareWithPerson(rest, flags2, output) {
  ...
  if (!who || rest.length !== 1) {
    output.error("Usage: ideaspaces share person <email|@handle> [--grade explore|fork|collaborate] [--history] [--repo <url>]");
    return 1;
  }
  ...
  if (!config)
    return 1;
  const target = await resolveTarget(repoFlag(flags2), config, output);
  if (!target)
    return 1;
...
async function shareWithTeam(rest, flags2, output) {
  ...
  if (!hostname || rest.length !== 1) {
    output.error("Usage: ideaspaces share team <hostname> [--grade explore|fork|collaborate] [--repo <url>]");
    return 1;
  }
  if (flags2.history) {
...
async function listProductAccess(rest, flags2, output) {
  ...
  for (const invite of invites) {
    lines.push(`  ${invite.invited_email.padEnd(24)} invited (${invite.grade}${invite.share_history ? " + history" : ""})` + invitationDeliverySummary(invite));
  }
...
async function removeProductAccess(rest, flags2, output) {
  ...
  if (standing?.direct_capabilities.length) {
    ...
    let effectiveCapabilitiesUnavailable = null;
    try {
      const after = await listPersonShares(config, target);
      effectiveCapabilities = after.standings.find((row) => row.user_id === standing.user_id)?.effective_capabilities ?? [];
    } catch (err) {
    ...
    const remains = effectiveCapabilities === null ? " Direct access was removed, but remaining access could not be checked." : effectiveCapabilities.length ? ` ${recipientName(standing)} still has ${capabilitySummary(effectiveCapabilities)} through another path.` : "";
    output.result(verified, (result.status === "removed" ? `Removed direct access for ${recipientName(standing)}.` : `Direct access was already removed for ${recipientName(standing)}.`) + remains);
    return 0;
  }
  const invites = invitesResult.status === "fulfilled" ? invitesResult.value.invites : [];
...
async function resendInvitation(rest, flags2, output) {
  const email = rest[0];
  ...
  const invite = collection.invites.find((row) => row.invited_email.toLowerCase() === selector.email.toLowerCase());
  if (!invite) {
  ...
  if (!invite.can_resend) {
    const wait = invite.resend_retry_after_seconds;
    output.error(wait && wait > 0 ? `That invitation cannot be resent yet. Try again in ${wait} second${wait === 1 ? "" : "s"}.` : "That invitation cannot be resent right now.");
    return 1;
  }
  const resent = await resendPersonShareInvite(config, target, invite.invite_id);
  output.result(resent, resent.delivery_status === "failed" ? `Tried to resend the invitation to ${resent.invited_email}, but delivery failed${resent.delivery_error ? `: ${resent.delivery_error}` : "."}` : `Resent the ${resent.grade} invitation to ${resent.invited_email}.`);
  return 0;
}
async function setHistory(rest, flags2, output) {
  const who = rest[0];
  ...
  if (result.status === "not_direct") {
    output.result(result, enabled ? `${recipientName(standing)} no longer has a direct person share, so hosted history was not enabled.` : `No direct hosted history remained for ${recipientName(standing)}. Other access was not changed by this command.`);
    return 0;
  }
...
async function run(sub, rest, flags2, output, yes) {
  try {
    switch (sub) {
      ...
      case "history":
        return await setHistory(rest, flags2, output);
      case "remove":
        if (rest.length === 2 && rest[0]?.startsWith("repo_")) {
          output.error("Repository-member removal was retired. Use `ideaspaces share remove <email|@handle|team:hostname>`.");
          return 1;
        }
...
var shareCommand = {
  ...
  async run(args2, flags2, global2) {
    const output = createOutput(global2);
    const [sub, ...rest] = args2;
    const withRepo = global2.repo === void 0 ? flags2 : { repo: global2.repo, ...flags2 };
    return run(sub ?? "", rest, withRepo, output, global2.yes === true);
  }
};
    ...
    for (const input of paths) {
      ...
      if (!scope) {
        try {
          ...
          const folderRoot = [...knownFolderRoots, ...explicitRoot ? [explicitRoot] : []].find((root) => contains(root, absolute));
