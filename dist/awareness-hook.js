...
var findSpaceRoot = findNearestAgent;
...
async function collectDocDependencies(repoRoot, docDir) {
  ...
  const start = resolve2(root, docDir);
  const out = [];
  ...
  if (git && gs && repoRoot) {
    ...
    if (signals.length) {
      ...
      for (const s of signals.slice(0, MAX_DRIFT)) {
        lines.push(
          s.kind === "stale" ? `  ${s.doc} \u2014 \`${s.newestCode}\` was committed after the doc` : `  ${s.doc} \u2014 references missing path(s): ${s.missing.join(", ")}`
        );
      }
